...
pub enum Agent {
    ...
    Claude,
    /// Cursor IDE — reads .cursor/rules/*.mdc (MDC format with frontmatter)
    Cursor,
    /// OpenAI Codex CLI — reads AGENTS.md
    Codex,
    /// Aider — reads .aider.conf.yml or CONVENTIONS.md
    Aider,
    /// Raw output — no agent wrapper (current default behavior)
    Raw,
}
...
const SOLIDITY_GUARDRAILS: &str = r#"- Do not reference Solidity features from versions higher than the pragma specifies.
- Do not invent EIPs or precompile addresses — verify they exist on the target chain.
- Do not hallucinate storage slot layouts or ABI encoding details."#;
...
fn format_copilot(content: &str, _language: &str) -> String {
    // Copilot reads .github/copilot-instructions.md as system-level context.
    // Keep concise. No frontmatter needed.
    format!(
...
fn format_claude(content: &str, _language: &str) -> String {
    ...
    // Supports nested CLAUDE.md in subdirectories for overrides.
    format!(
        r#"<!-- Generated by promptctl — Claude Code instructions -->
<!-- Regenerate: promptctl emit claude -->
<!-- Supports nested CLAUDE.md in subdirectories for scope overrides -->
...
fn format_codex(content: &str, _language: &str) -> String {
    // Codex reads AGENTS.md with task-oriented phrasing.
    format!(
        r#"<!-- Generated by promptctl — OpenAI Codex agent instructions -->
<!-- Regenerate: promptctl emit codex -->
...
<!-- Regenerate: promptctl emit aider -->
...
pub fn install_agent_hooks(
    ...
    match agent {
        Agent::Claude => install_claude_hooks(project_root, force),
        Agent::Cursor => install_cursor_hooks(project_root, languages, force),
        Agent::Copilot => install_copilot_hooks(project_root, languages, force),
...
fn install_claude_hooks(project_root: &Path, force: bool) -> Result<Vec<HookFile>, AgentError> {
    ...
    write_hook_file(&session_script_path, CLAUDE_SESSION_START_SCRIPT, force)?;
    ...
    write_hook_file(&pre_write_path, CLAUDE_PRE_WRITE_SCRIPT, force)?;
...
GUIDELINES=$(promptctl generate --agent raw --size compact --no-guardrails 2>/dev/null)
...
<!-- Generated by promptctl hooks install cursor -->
<!-- Regenerate: promptctl hooks install cursor --force -->

$(promptctl show {lang} --role developer 2>/dev/null || echo "Run 'promptctl show {lang}' to see guidelines")
"#,
            lang = lang,
            ext = info.ext,
            globs = info.globs,
        ...
        written.push(HookFile {
            path,
            description: format!("{lang} guidelines for {ext} files", ext = info.ext),
        });
    }
...
<!-- These instructions apply only when Copilot works on {ext} files -->
...
"#,
        ...
        written.push(HookFile {
            path,
            description: format!("{lang} instructions for {ext} files", ext = info.ext),
        });
    }
